...
import ChatGptIcon from '../../assets/Ellipse 1.svg';
...
const ChatPage = () => {
  ...
  useEffect(() => {
    ...
    const getInitialAnswer = async () => {
      await simulateTyping("Hello! How can I assist you today?", 'CHAT GPT');
    };
  ...
  const handleSendMessage = async () => {
    ...
    setMessages((prev) => [...prev, { text: inputMessage, source: 'YOU', key: Date.now() }]);

    await simulateTyping('', 'CHAT GPT');
    ...
    try {
      ...
      if (apiResponse && apiResponse.message && apiResponse.message.trim()) {
        setMessages((prev) => [
          ...prev,
          { text: apiResponse.message.trim(), source: 'CHAT GPT', key: Date.now() },
        ]);
      }
    } catch (error) {
  ...
  return (
    <div>
      <header className="header">
        ...
        <div className="title">Chat with GPT</div>
      </header>
        ...
        <div className="chat-container">
          {messages.map((message, index) => (
            (message.text && message.text.trim()) && (
              <div key={message.key} className={message.source === 'YOU' ? 'you' : 'gpt'}>
                {message.source === 'YOU' && (
                ...
                {message.source === 'CHAT GPT' && (
                  <div className="gpt-avatar">
                    <img src={ChatGptIcon} alt="" />
                    <div className="chatgpt-title">CHAT GPT</div>
                  </div>
                )}
                {message.text && message.text.trim()}
              </div>
